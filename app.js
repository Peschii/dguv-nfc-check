...
function logWalkCheck(entry) {
  ...
  state.walkLog = dedupeWalkRows([row, ...state.walkLog]).slice(0, 500);
  persistWalkLog();
  renderWalkLog();
...
function renderWalkLog() {
  state.walkLog = dedupeWalkRows(state.walkLog);
...
function getWalkLogStats() {
  return dedupeWalkRows(state.walkLog).reduce(
    (stats, row) => {
...
function analyzeReportInput() {
  reportRows = dedupeWalkRows(parseReportText(els.reportInput.value));
  renderReport();
}
...
function dedupeWalkRows(rows) {
  const seen = new Set();
  const cleaned = [];
  for (const row of rows || []) {
    const key = normalizeKnownTagPrefix(normalizeTag(row.tagId));
    if (!key || seen.has(key)) continue;
    seen.add(key);
    cleaned.push({ ...row, tagId: key });
  }
  return cleaned;
}

function renderReport() {
  reportRows = dedupeWalkRows(reportRows);
...
function getRowsStats(rows) {
  return dedupeWalkRows(rows).reduce(
    (stats, row) => {
...
async function downloadReportPng() {
  ...
  const stats = getRowsStats(reportRows);
  const inspectors = summarizeReportField("inspector");
  const labs = summarizeReportField("lab");
  ...
  ctx.fillText(`Prüfer: ${inspectors}`, 60, 150);
  ctx.fillText(`Labor: ${labs}`, 60, 184);
  ...
  if (reportRows.length > previewRows.length) {
    ctx.fillText(`... plus ${reportRows.length - previewRows.length} weitere`, 60, 510 + previewRows.length * 30);
  }
...
function summarizeReportField(field) {
  const values = [...new Set(reportRows.map((row) => String(row[field] || "").trim()).filter(Boolean))];
  if (!values.length) return "-";
  if (values.length <= 3) return values.join(", ");
  return `${values.slice(0, 3).join(", ")} + ${values.length - 3} weitere`;
}
...
function loadWalkLog() {
  try {
    ...
    return Array.isArray(parsed) ? dedupeWalkRows(parsed) : [];
  } catch {
    return [];
  }
}
